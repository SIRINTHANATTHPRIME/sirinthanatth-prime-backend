...
class UserRegistrationFlow(BaseModel):
    ...
    อัปเกรด: Anti-XSS Sanitization, KYC Auto-clean, และ Strict/Forbid Extra Fields
    """
    # forbid = ห้ามมีฟิลด์ขยะหรือฟิลด์ที่ไม่ได้ประกาศไว้หลุดเข้ามาเด็ดขาด
    ...
    @field_validator('shop_name', 'product_type')
    @classmethod
    def sanitize_text_inputs(cls, v: str) -> str:
        """ป้องกัน XSS และ Prompt Injection โดยลบแท็ก HTML และสัญลักษณ์อันตรายทิ้ง"""
    ...
    def validate_kyc_format(cls, v: str) -> str:
        """เคลียร์ขีดกลางและช่องว่างทิ้งอัตโนมัติ ก่อนเช็กว่าเป็นตัวเลขล้วนหรือไม่ (เพิ่ม UX ให้ลูกค้า)"""
