...
def create_exclusive_invite(min_topup_thb: int = 100) -> dict:
    ...
    ให้เรียกใช้ฟังก์ชันนี้ทันทีเมื่อท่านประธานสั่งให้ "สร้างลิงก์", "ออกคำเชิญ", หรือ "เชิญคนเข้าใช้งาน"
    
    Args:
        min_topup_thb: จำนวนเงินขั้นต่ำที่ผู้ถูกเชิญต้องเติมเข้า Wallet เพื่อเปิดระบบ (ค่าเริ่มต้นคือ 100)
    """
    try:
        ...
        # 1. บันทึกลงฐานข้อมูล (แยกสิทธิ์การใช้งาน ไม่ให้เข้าถึงเลขาได้)
        if supabase:
...
class CeoSecretaryWorker:
    ...
    อัปเกรด: Autonomous Function Calling, Truth-Based Reasoning, Full System Controller
    """
    ...
    def __init__(self):
        ...
        1. 🛠️ Autonomous Function Calling: คุณมีเครื่องมือ (Tools) ให้เรียกใช้ เช่น การสร้างลิงก์ VVIP หากคำสั่งเกี่ยวข้องกับการสร้างลิงก์ ให้ประเมินและเรียกใช้ Tool ทันที
        2. 🧠 Truth-Based Strategy: เสนอแผนธุรกิจที่อิงจากข้อมูลจริง ทำได้จริง ไม่เพ้อฝัน
        3. 💻 Full-Stack System Controller: เขียนโค้ดแก้ไขไฟล์ได้ทุกไฟล์ โดยใช้ [UPDATE_SYSTEM_FILE: path]...[/UPDATE_SYSTEM_FILE]
        4. 🏢 Swarm Commander: สั่งงานแผนกอื่นด้วย [DELEGATE: WORKER_NAME] คำสั่ง...
        
        🚨 กฎเหล็ก: ท้ายการอัปเดตไฟล์ระบบ คุณต้องพิมพ์ [REQUIRE_APPROVAL] เสมอ
    ...
    async def process_ceo_command(self, message: str, file_path: str = None, file_type: str = None) -> dict:
        ...
        # 1. ระบบประมวลผลคำสั่งอนุมัติ
        if message.startswith("ACTION:APPROVE:"): return await self._execute_approved_plan(message)
        elif message.startswith("ACTION:REJECT:"): return {"type": "text", "text": f"❌ รับทราบครับท่านประธาน แผนงานและโค้ดถูกระงับ 100% ครับ"}
        elif message.startswith("ACTION:MODIFY:"): return {"type": "text", "text": f"📝 รับทราบครับท่านประธาน รบกวนสั่งการจุดที่ต้องการให้ปรับปรุง ผมจะรื้อโครงสร้างให้ใหม่ทันทีครับ"}
        ...
        try:
            # 3. 🧠 ดึงข้อมูลความจำองค์กรแบบขนาน
            ...
            # =========================================================================
            # 5. ⚡ THE AUTONOMOUS LOOP: สั่งรัน AI และประมวลผล Function Calling (อัจฉริยะ)
            # =========================================================================
            logger.info("⏳ [CEO Secretary]: กำลังคิดวิเคราะห์กลยุทธ์มหาภาคและสถาปัตยกรรมระบบ...")
            
            # 🔧 ยัดอาวุธ (Tools) ใส่มือ AI
            ...
            # 🤖 วนลูปหาก AI ร้องขอการใช้งาน Tool (Function Calling)
            while response.function_calls:
                for fn_call in response.function_calls:
                    logger.info(f"🛠️ [Function Calling Activation]: AI ตัดสินใจเรียกใช้ -> {fn_call.name}")
                    ...
                    if fn_call.name == "create_exclusive_invite":
                        ...
                        # นำคำร้องขอเดิมของ AI ใส่กลับเข้า Context
                        content_to_send.append(response.candidates[0].content)
                        
                        # ส่งผลลัพธ์จากฟังก์ชัน (Result) กลับไปให้ AI อ่านและสรุป
                ...
                # 🔁 ยิงกลับไปให้ AI สรุปข้อมูลอีกครั้งด้วยเนื้อหาที่ครบถ้วนแล้ว
                logger.info("⏳ [CEO Secretary]: ได้ข้อมูลจากฐานข้อมูลแล้ว กำลังประพันธ์คำตอบ...")
                response = await asyncio.to_thread(
                    self.client.models.generate_content,
                    model=self.model_name,
                    contents=content_to_send,
            ...
            return {"type": "text", "text": "ขออภัยครับท่านประธาน โปรเจกต์นี้มีข้อมูลลึกซึ้งมาก รบกวนสั่งการให้ผมโฟกัสแก้ไขทีละไฟล์นะครับ"}
        except Exception as e:
            logger.error(f"⚠️ [CEO Secretary Error]: {e}", exc_info=True)
            return {"type": "text", "text": f"ขออภัยครับท่านประธาน เกิดข้อผิดพลาดทางวิศวกรรม ({str(e)[:50]}) ผมส่ง Log ให้ฝ่ายระบบแล้วครับ"}
